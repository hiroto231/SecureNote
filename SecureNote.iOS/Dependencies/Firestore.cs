...
[assembly: Dependency(typeof(SecureNote.iOS.Dependencies.Firestore))]

namespace SecureNote.iOS.Dependencies
{
    public class Firestore :IFirestore
    {
        ...
        public bool InsertSubscription(Subscription subscription)
        {
            try
            {
                ...
                Firebase.CloudFirestore.Firestore.SharedInstance.GetCollection("Subscription").AddDocument(subscriptionDocument);
                return true;
            }
            catch(Exception ex)
            {
